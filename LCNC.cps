...
properties = {
	...
	spindleDwellTime: {title:"Spindle dwell Time", description:"Time to wait for the spindle to get up to target speed", group:machineConfigGroup, type:"number", value: 1},
	...
	probeFastSpeed: {title: "Fast probing speed (mm/min)", description: "Fast probing speed (mm/min)", type:"number", group:probingGroup, value: 500.0},

	probeSlowSpeed: {title: "Slow probing speed (mm/min)", description: "Slow probing speed (mm/min)", type:"number", group:probingGroup, value: 50.0},

	probeSlowDistance: {title: "Slow probe distance (mm)", description: "Slow probe distance (mm)", type:"number", group:probingGroup, value: 2},

	// properties to control tool setter functions
	etsTolerance: {title: "Tolerance for ETS checks (mm)", description: "Tolerance allowed for tool lengths for ETS checks", type:"number", group:etsGroup, value: 0.1},

	etsDiameterLimit: {title: "ETS diameter limit (mm)", descriptopn: "Tools larger than this will not be checked by the tool setter", type: "number", group:etsGroup, value: 10},
...
function UseToolWithETS(tool)
{
	...
	if (tool.diameter > getProperty("etsDiameterLimit"))
	{
...
function CheckCurrentTool(tool)
{
	if (UseToolWithETS(tool))
	{
		...
		writeBlock(gFormat.format(37), "P" + getProperty("etsTolerance"));
...
function onOpen() 
{
	...
	writeBlock("#<_probeFastSpeed>=", getProperty("probeFastSpeed"));
	writeBlock("#<_probeSlowSpeed>=", getProperty("probeSlowSpeed"));
	writeBlock("#<_probeSlowDistance>=", getProperty("probeSlowDistance"));
...
//				UserRetract(getProperty("retractOnWorkPlaneChange"), "new work plane", false);
		...
		(tool.clockwise != getPreviousSection().getTool().clockwise)) 
	{
		...
		else 
		{
			...
			onDwell(getProperty("spindleDwellTime"));
		}
	} // set spindle
